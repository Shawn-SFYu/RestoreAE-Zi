...
def rect_painting(
    ...
    num_strokes=10,
    stroke_range=(1, 50),
    stroke_intensity_range=(-255 / 5, 255 / 5),
):
...
def add_noise(image, density=0.0005, kernel_size=15):
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size))
    noise1 = np.random.choice(
        [0, 255 / 4], size=image.shape[:2], p=[1 - density, density]
    ).astype(np.float32)
    noise1 = cv2.dilate(noise1, kernel)
    noise2 = np.random.choice(
        [0, 255 / 4], size=image.shape[:2], p=[1 - density, density]
    ).astype(np.float32)
...
if __name__ == "__main__":
    ...
    img = (
        (img.astype(np.float) - np.min(img)) * 255 / (np.max(img) - np.min(img))
    ).astype(np.uint8)
